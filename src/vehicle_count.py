...
ultimi_avvistamenti = (
    dfstream.selectExpr("CAST(key AS STRING)", "CAST(value AS STRING)")
    .select(
        from_csv(col("value"), schema, options).alias("data"), col("key").alias("batch")
    )
    .select("batch", "data.*")
)

conteggio = (
    ultimi_avvistamenti.groupBy("batch", "tratto")
    .count()
...
print("\n\n\nStarting...\n\n\n")
query = (
    conteggio.writeStream.format("console")
    .option("numRows", 200)
